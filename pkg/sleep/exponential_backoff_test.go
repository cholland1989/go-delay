...
func ExampleExponentialBackoffWithContext() {
	...
	for attempt := 0; attempt < 5; attempt++ {
		...
		ExponentialBackoffWithContext(ctx, time.Second, 2.0, 0.5, attempt)
	}
	// Output:
}
	...
	for name, params := range map[string]struct {
		...
		test.Run(name, func(test *testing.T) {
			...
			for _, ctx := range []context.Context{nil, context.Background()} {
				timestamp := time.Now()
				ExponentialBackoffWithContext(ctx, params.duration, params.multiplier, 0.0, params.attempt)
				delay := time.Since(timestamp)
				if delay < params.expected {
					test.Fatalf("expected %v, got %v", params.expected, delay)
				}
...
func TestExponentialBackoffWithContext_WithCancel(test *testing.T) {
	...
	cancel()
	timestamp := time.Now()
	ExponentialBackoffWithContext(ctx, time.Second, 1.0, 0.0, 0)
	delay := time.Since(timestamp)
